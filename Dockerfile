# STEP 1: Build executable binary
FROM golang:1.9 AS builder
# Copy project into image
COPY . $GOPATH/src/github.com/chandan4u/go-graphql
# Set working directory to /go-graphql which contains main.go
WORKDIR $GOPATH/src/github.com/chandan4u/go-graphql
# Build the binary
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -a -installsuffix cgo -o /go/bin/api

# STEP 2: Build a small image start from scratch
FROM scratch
# Expose port 4000
EXPOSE 4000
# Copy our static executable
COPY --from=builder /go/bin/api .
# Command is run when container starts
ENTRYPOINT ["./api"]
